...
export default function AdminPanel({ token, user, onLogout }: AdminPanelProps) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#111120] flex" id="admin-panel-container">
      {/* 1. SIDEBAR — Dataflow-inspired design */}
      <aside className="w-64 bg-[#1A1A2E] dark:bg-[#0A0A15] text-[#C8C4BC] flex flex-col shrink-0 border-r border-[#333348]" id="admin-sidebar">
        <div className="p-5 border-b border-[#333348] flex items-center gap-3">
          <img src="/logo/dw-mark.png" alt="DawnWire" className="w-9 h-9 object-contain rounded-lg bg-[#C9A96E]/10 p-1 shadow-lg" draggable={false} />
          <div>
            <h1 className="font-['Playfair_Display',serif] font-bold text-[#F0EDE8] tracking-tight leading-none text-sm">DawnWire</h1>
            <span className="text-[10px] text-[#8A857C] font-medium">Super Admin Panel</span>
          </div>
        </div>
          ...
          {[
            ...
            <              motion.button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all relative overflow-hidden ${
                activeMenu === item.key
                  ? 'bg-[#252538] text-[#D4B87A] shadow-sm'
                  : 'text-[#C8C4BC] hover:bg-[#252538]/50 hover:text-[#F0EDE8]'
              }`}
              whileHover={{ x: 4 }}
              whileTap={{ scale: 0.98 }}
            ...
            >
              ...
              {activeMenu === item.key && (
                <motion.div
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-[#D4B87A] rounded-r-full"
        ...
        {/* User box */}
        <div className="p-3 border-t border-[#333348] bg-[#1A1A2E]/80">
          <div className="flex items-center gap-2.5 px-1">
            <img
              ...
              className="h-7 w-7 rounded-full border border-[#444460] object-cover shrink-0"
              onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-[#F0EDE8] truncate leading-tight">{user.name}</p>
              <span className="text-[10px] text-[#D4B87A] font-medium capitalize">{user.role.replace('_', ' ')}</span>
            </div>
            <button
              onClick={onLogout}
              title="Sign Out"
              className="text-[#8A857C] hover:text-red-400 p-1.5 hover:bg-[#252538]/50 rounded-lg transition-all shrink-0"
      ...
      <main className="flex-1 flex flex-col min-w-0" id="admin-main-content">
        ...
        <header className="bg-white dark:bg-[#1C1C2E]/90 border-b border-[#E5E1DC] dark:border-[#333348] h-16 flex items-center justify-between px-6 shrink-0 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <h2 className="font-['Playfair_Display',serif] font-bold text-[#1A1A2E] dark:text-[#F0EDE8] capitalize tracking-tight text-lg" id="header-menu-title">
              {activeMenu === 'seo' ? 'SEO Engine' : activeMenu.replace('_', ' ')}
            </h2>
            <span className="text-[10px] text-[#8A857C] uppercase tracking-wider font-medium hidden sm:block">Super Admin Panel</span>
          </div>
          <div className="flex items-center gap-3">
            <a
              ...
              className="text-xs bg-[#FAF8F5] dark:bg-[#252538] text-[#374151] dark:text-[#C8C4BC] hover:bg-[#F5F0EB] dark:hover:bg-[#333348] hover:text-[#1A1A2E] dark:hover:text-[#F0EDE8] px-3.5 py-2 rounded-xl font-medium transition-all flex items-center gap-1.5 border border-[#E5E1DC] dark:border-[#444460]"
            >
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#FAF8F5]/50 dark:bg-[#111120]" id="admin-workspace">
          {authExpired && (
            <div className="mb-6 p-4 bg-[#FAF8F5] dark:bg-[#252538] border border-[#C9A96E] dark:border-[#D4B87A] rounded-2xl flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <LogOut className="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0" />
                <div>
                  <h4 className="text-xs font-bold text-[#1A1A2E] dark:text-[#F0EDE8]">Session Expired</h4>
                  <p className="text-[11px] text-[#6B7280] dark:text-[#8A857C]">Your admin authentication token has expired. Please log in again to manage records.</p>
                </div>
              </div>
              <button
                onClick={onLogout}
                className="px-4 py-2 bg-[#1A1A2E] dark:bg-[#D4B87A] hover:bg-[#2D2D44] dark:hover:bg-[#C9A96E] text-[#C9A96E] dark:text-[#1A1A2E] font-bold text-xs rounded-xl shadow-sm transition-all shrink-0 cursor-pointer"
              >
                Log In Again
              </button>
            </div>
          ...
          {activeMenu === 'activity' && (
            <div className="space-y-6" id="admin-workspace-activity">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-['Playfair_Display',serif] font-bold text-[#1A1A2E] dark:text-[#F0EDE8]">Activity Feed & User Insights</h2>
              </div>
              <ActivityHeatmapD3 />
              <ActivityFeedTab />
            </div>
          ...
          {/* BRANDS VIEW */}
          {activeMenu === 'brands' && (
            <div className="bg-white dark:bg-[#1C1C2E]/50 rounded-2xl br-card border border-[#E5E1DC] dark:border-[#333348]/50 shadow-sm p-6">
          ...
          {/* MEDIA GALLERY VIEW */}
          {activeMenu === 'media' && (
            <div className="bg-white dark:bg-[#1C1C2E]/50 rounded-2xl br-card border border-[#E5E1DC] dark:border-[#333348]/50 shadow-sm p-6" id="admin-workspace-media">
              <MediaGallery
                items={media}
                onDelete={async (id) => {
          ...
          {activeMenu === 'clusters' && (
            <div className="bg-white dark:bg-[#1C1C2E]/50 rounded-2xl br-card border border-[#E5E1DC] dark:border-[#333348]/50 shadow-sm overflow-hidden" id="admin-workspace-clusters">
              <div className="p-4 bg-[#FAF8F5] dark:bg-[#111120] border-b border-[#E5E1DC] dark:border-[#333348]/50 flex items-center justify-between">
                <span className="text-xs font-bold text-[#374151] dark:text-[#C8C4BC]">Topic Clusters ({topicClusters.length})</span>
                <span className="text-[10px] text-[#9CA3AF] dark:text-[#8A857C]">Pillar pages with cluster content for SEO topic authority</span>
              </div>
              <div className="p-4 space-y-4">
                {topicClusters.length === 0 ? (
                  <p className="text-xs text-[#9CA3AF] dark:text-[#8A857C] p-8 text-center">No topic clusters created yet. Clusters group a pillar page with related content for SEO topical authority.</p>
                ) : (
                  topicClusters.map((cluster: any) => (
                    <div key={cluster.id} className="border border-[#E5E1DC] dark:border-[#333348] rounded-xl p-4 hover:border-[#246BFF]/50 transition-all">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h4 className="font-bold text-[#1A1A2E] dark:text-[#F0EDE8] text-sm">{cluster.name}</h4>
                            <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${cluster.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-[#FAF8F5] text-[#6B7280] dark:text-[#9CA3AF]'}`}>
                              {cluster.status}
                            </span>
                          </div>
                          <p className="text-[11px] text-[#6B7280] dark:text-[#9CA3AF] mt-1 line-clamp-2">{cluster.description}</p>
                          <div className="flex items-center gap-4 mt-2 text-[10px] text-[#9CA3AF] dark:text-[#8A857C]">
