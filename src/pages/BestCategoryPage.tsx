            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {withProducts.map(({ cat, count }) => (
                ...
                >
                  ...
                  <span className="inline-flex items-center gap-1 mt-4 text-xs font-bold text-blue-600 dark:text-blue-400">View ranking â†’</span>
                </a>
              ))}
            </div>
          )}
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-24">
      ...
      <div className="bg-gradient-to-br from-[#0A1F44] via-[#12307a] to-[#1a3f9e] text-white">
        <div className="max-w-7xl mx-auto px-4 py-14 sm:py-18">
          <p className="text-xs font-bold text-amber-400 uppercase tracking-widest mb-2">DawnWire Roundup Â· {list.length} tested products</p>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10 space-y-12">
        {/* Winner Podium */}
        <section>
          <h2 className="text-2xl font-extrabold flex items-center gap-2 mb-4">ðŸ† Our #1 Pick: {winner.title}</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-4 flex justify-center">
                {winner.images?.[0] || (winner as any).product_image ? (
                  ...
                  <div className="w-56 h-56 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-6xl">ðŸ†</div>
                )}
              </div>
              <div className="lg:col-span-8 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-bold bg-[#0A1F44] text-amber-400 px-3 py-1.5 rounded-xl">
                    â˜… {toPrice(winner.editorScore) || (toPrice(winner.rating) * 2) || 'â€”'} / 10
                  </span>
                  {toPrice(winner.rating) > 0 && <span className="text-xs font-bold text-slate-500 dark:text-slate-400">â˜… {toPrice(winner.rating)} ({toPrice(winner.reviewCount)} reviews)</span>}
                </div>
                <AffiliateCTA
                  ...
                  variant="deal"
                  size="lg"
                  label="Buy Now â€” Check Price on Amazon"
        ...
        {list.length > 1 && (
          <section className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {list.slice(1, 3).map((p: any, idx: number) => {
              const label = p.id === topRated?.id ? 'â­ Top Rated' : p.id === bestValue?.id ? 'ðŸ’° Best Value' : idx === 0 ? 'ðŸ¥ˆ Runner Up' : 'ðŸ¥‰ Also Great';
              return (
                <div key={p.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {toPrice(p.rating) > 0 && <span className="text-xs font-bold text-amber-500">â˜… {toPrice(p.rating)}</span>}
                  </div>
        ...
        {list.length > 1 && (
          <section>
            <h2 className="text-2xl font-extrabold mb-4">ðŸ“Š Full Comparison</h2>
                    ...
                    {list.map((p: any, idx: number) => (
                      <tr key={p.id} className="border-b border-slate-100 dark:border-slate-800/60 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        ...
                        <td className="px-4 py-3 font-black text-amber-600 dark:text-amber-400">
                          {toPrice(p.editorScore) || (toPrice(p.rating) * 2) || 'â€”'}
                        </td>
                        <td className="px-4 py-3">{toPrice(p.rating) ? `â˜… ${toPrice(p.rating)}` : 'â€”'}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className="inline-flex items-center gap-2 justify-end">
                            ...
                            <a href={`/products/${p.slug}`} className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline shrink-0">Review â†’</a>
                          </span>
                        </td>
                      </tr>
                    ))}
