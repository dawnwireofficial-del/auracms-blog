...
export const SideBySideComparisonModal: React.FC<SideBySideComparisonProps> = ({
  ...
  // Derived head-to-head comparison metrics
  const priceDiff = Math.abs(productA.currentPrice - productB.currentPrice);
  const cheaperProduct = productA.currentPrice < productB.currentPrice ? 'A' : productA.currentPrice > productB.currentPrice ? 'B' : 'EQUAL';
  const ratingWinner = productA.rating > productB.rating ? 'A' : productA.rating < productB.rating ? 'B' : 'EQUAL';
