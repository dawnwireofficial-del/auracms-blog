...
export const PriceHistoryTracker: React.FC<PriceHistoryTrackerProps> = ({ product }) => {
  ...
  const [targetPrice, setTargetPrice] = useState<number>(Math.round(product.currentPrice * 0.85));
  ...
  useEffect(() => {
    const loadHistory = async () => {
      setIsLoading(true);
      const data = await getProductPriceHistory(product.id, product.currentPrice);
      setHistory(data);
      setIsLoading(false);
    };
    loadHistory();
  ...
  const minPrice = prices.length > 0 ? Math.min(...prices) : product.currentPrice;
  const maxPrice = prices.length > 0 ? Math.max(...prices) : product.currentPrice;
  const avgPrice = prices.length > 0 ? Math.round((prices.reduce((a, b) => a + b, 0) / prices.length) * 100) / 100 : product.currentPrice;
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={history} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(val: number) => [`$${val.toFixed(2)}`, 'Amazon Price']}
