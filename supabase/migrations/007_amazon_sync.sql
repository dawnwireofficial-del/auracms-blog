-- Amazon Product Synchronization Schema
...
CREATE TABLE IF NOT EXISTS amazon_sync_status (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id UUID NOT NULL REFERENCES product_reviews(id) ON DELETE CASCADE,
  asin TEXT NOT NULL,
...
CREATE INDEX idx_amazon_sync_status_product_id ON amazon_sync_status(product_id);
CREATE INDEX idx_amazon_sync_status_asin ON amazon_sync_status(asin);
CREATE INDEX idx_amazon_sync_status_status ON amazon_sync_status(sync_status);
CREATE INDEX idx_amazon_sync_status_next_sync ON amazon_sync_status(next_sync_at) WHERE next_sync_at IS NOT NULL;
CREATE INDEX idx_amazon_sync_status_priority ON amazon_sync_status(priority DESC);
...
CREATE INDEX idx_amazon_price_history_product ON amazon_price_history(product_id);
CREATE INDEX idx_amazon_price_history_asin ON amazon_price_history(asin);
CREATE INDEX idx_amazon_price_history_created ON amazon_price_history(created_at DESC);

-- 4. Amazon Sync Logs
CREATE TABLE IF NOT EXISTS amazon_sync_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  batch_id UUID,
  product_id UUID REFERENCES product_reviews(id) ON DELETE SET NULL,
  asin TEXT,
...
CREATE INDEX idx_amazon_sync_logs_batch ON amazon_sync_logs(batch_id);
CREATE INDEX idx_amazon_sync_logs_product ON amazon_sync_logs(product_id);
CREATE INDEX idx_amazon_sync_logs_created ON amazon_sync_logs(created_at DESC);
