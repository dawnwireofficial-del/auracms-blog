...
### Session — Homepage data outage fixed end-to-end + full-site audit (this session)
- **Root cause of missing trust icons/banners/products on the homepage = two stacked failures**: (1) the MySQL app user lost its privileges (SHOW GRANTS = only USAGE; live API 500s on `homepage_hero_slides`, `posts`, `brands`; products/categories silently empty), and (2) every deploy's prebuild regenerated `public/data/{catalog,homepage,categories,brands}.json` from the FROZEN Supabase project → wrote `product_count: 0` into every catalog from commit `7f6dc3c` (Sep 4) onward, and the products endpoint serves from that static catalog → rails/deals went blank. (Trust icons themselves were fine — static files, 200.)
- **Fixed**: restored last-good snapshots (836 products / 34 categories / 571 brands / 20 homepage picks from `ad248659`); rewrote `scripts/generate-product-catalog.ts` to read **MySQL** (real backend) with a no-wipe guard (aborts if a run yields 0 while files hold data — verified).
- **DB grants are STILL missing at the Hostinger level** — the app user has no table privileges; the API now degrades gracefully instead of 500ing (hero/posts/brands/deals/affiliate return fallback data), and detail endpoints (post/product slug) serve from the static catalog. **To fully restore live DB data, grant privileges in Hostinger hPanel** (SQL in the “Site Revive” note below; `GRANT ... ON u916810702_dawnwire.* TO 'u916810702_dawnwire'@'%'` + Vercel IP rows). Vercel's IP also rotates — wildcard `'%'` covers it.
- **Deploy pipeline permanently fixed**: GitHub Actions `VERCEL_TOKEN` secret was never authorized for the new project → every run failed since Sept 4 (15+ consecutive). Deploys now run locally via the **dawnwire Vercel CLI session** (auth.json read by `scripts/read-vercel-token.cjs`) → `scripts/vercel-deploy.mjs` (gitSource). **Never deploy with the Bisma Vercel account** — this machine previously had it; `vercel logout` + relogin with the dawnwire account fixed it.
- **Static catalog reachable in serverless**: `includeFiles` is rejected as non-string, so the lambda reads `/data/*.json` from its own CDN via async fetch fallback (`server/api-cache.ts` `readStaticCatalogRemote`; `vercel.json` filesystem handler serves `public/data` at `/data/*`).
- **Full-site audit tooling added** (`scripts/audit-pages.mjs` headless-Chrome sweep: console errors / failed HTTP / broken images / content markers; `scripts/shoot-pages.mjs` desktop+mobile screenshots to `qa-shots/`; `scripts/diag-homepage.mjs`): homepage, products, categories, deals, guides, posts, brand pages, product detail, admin — all 200 with 0 console errors, 0 broken images after the fixes.
- **Post pages fixed for snapshot rows**: snapshot posts are lightweight (no content/tags/status) → `PostDetailPage` crashed on `content.split()`. Server normalizes snapshot rows to full camelCase Post shape (derives tags from slug so Buying Guides rails stay alive) and the client guards partial rows.
- Verified live: `/api/public/{products,categories,posts,brands,deals}` all 200 with real data; `homepage-hero` returns 200 empty (graceful) → homepage auto-hero fallback. Commits `8c8004c` → `ac32b8a` deployed via dawnwire CLI.

### Session — Extension imports fixed + SiteStripe long-link capture from real popover DOM (this session)
- **Import regression root-caused & fixed**: commit `8d7a34f` registered `social-background.js` (pin-only) as the ONLY service worker, so `IMPORT_PRODUCT` / `IMPORT_BATCH` / `IMPORT_FROM_URL` / queue / `TEST_CONNECTION` handlers living in `background.js` never ran → banners failed with “port closed”. Fix: `social-background.js` now `import './background.js'` so both handler sets run in one worker. Proven by `scripts/test-extension-worker.mjs` (all 5 message types handled) and `scripts/test-import-from-url.mjs` (9/9: URL import opens a background tab → content.js `EXTRACT_PRODUCT_DATA` returns live-DOM data incl. video + gallery → import POST → cloak).
- **SiteStripe long-link capture now matches the REAL T1 popover DOM** the user pasted (`amzn-ss-*`: Short/Full Link radios, Store/Tracking ID dropdowns `dawnwire-20`, “Copy affiliate link” button that GENERATES the URL only on click). `browser-extension/content.js` capture reworked:
  - `_installSiteStripeHooks()` — clipboard hook records exactly what SiteStripe writes via `navigator.clipboard.writeText` + re-scan after any `amzn-ss` click.
  - `_scanSiteStripeDom()` — caches every tagged Amazon URL currently on the page. (Bug caught: `[data-amzn-ss-*]` is INVALID CSS — throws `DOMException` in real Chrome too and silently kills the scan; removed.)
  - `primeSiteStripeFullLink(asin)` — when the popover is open, selects the **Full Link** radio then presses **Copy affiliate link** and polls for the generated deep URL (tag + linkCode/linkId/pd_rd_/ref_=as_li).
  - Import banner: passive 10s watch prefills the affiliate field the moment a deep link appears; the Import click actively primes an open popover (≤2.5s) before POSTing, so the product keeps the REAL SiteStripe long URL instead of a minted `?tag=`. Helpers exposed as `window.__dawnwireSs` for testing.
- **Verified**: `scripts/test-sitestripe-capture.mjs` (jsdom; installed dev-only via `npm i --no-save --no-package-lock jsdom`) — 21/21: classic toolbar textarea, bare-tag fallback, ASIN mismatch rejected, untagged/non-Amazon rejected, clipboard cache keeps deep-over-bare, popover prime (Full Link selected first, Copy pressed, deep URL resolved <2.5s), closed-popover fast-empty, pre-selected Full Link path.
- Note: `npx -y -p jsdom` does NOT make jsdom importable by an ESM script; install it into project `node_modules` instead (package.json/package-lock untouched).
- Pre-existing (NOT from this session, at HEAD `b252846`): `tsc --noEmit` reports `DashboardAnalytics.tsx(363,55)` TS2769 — file is unmodified in the working tree.
